...
/**
 * Carpeta física para imágenes de pruebas de multas personales
 * (fuera de `public` para que `vite build` no borre archivos).
 * Se sirve por HTTP como `/uploads/personalFineProves/<archivo>`.
 */
export const PERSONAL_FINE_PROVES_DIR = path.join(
  __dirname,
  "..",
  "..",
  "storage",
...
export const UPLOADED_PERSONAL_FINE_PROVES_PREFIX =
  "/uploads/personalFineProves/";

export function ensurePersonalFineProvesDirSync() {
  try {
    fs.mkdirSync(PERSONAL_FINE_PROVES_DIR, { recursive: true });
  } catch (e) {
    logger.error("ensurePersonalFineProvesDirSync: " + e);
  }
}

/**
 * Elimina un archivo previamente subido si su ruta web apunta a
 * `uploads/personalFineProves/` en disco. Ignora rutas externas y data URLs.
 */
export function deleteStoredProveFile(webPath) {
  if (!webPath || typeof webPath !== "string") return;
  const trimmed = webPath.trim();
  if (!trimmed.startsWith(UPLOADED_PERSONAL_FINE_PROVES_PREFIX)) return;

  const relativeName = trimmed
    .slice(UPLOADED_PERSONAL_FINE_PROVES_PREFIX.length)
    .replace(/^\/+/, "");
  if (
    !relativeName ||
  ...
  const absolute = path.join(PERSONAL_FINE_PROVES_DIR, relativeName);
  const resolvedDir = path.resolve(PERSONAL_FINE_PROVES_DIR);
  if (!absolute.startsWith(resolvedDir)) {
    logger.warn(
      "deleteStoredProveFile (personal): ruta fuera de storage/personalFineProves, omitido."
    );
    return;
  }
  try {
    ...
    logger.warn("deleteStoredProveFile (personal): " + e);
  }
}
...
export function renameProveFilesByFineNumber(files, fineNumber) {
  ...
  const resolvedRoot = path.resolve(PERSONAL_FINE_PROVES_DIR);
  const finalUrls = [];
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const fromAbs = path.join(PERSONAL_FINE_PROVES_DIR, f.filename);

    if (!sanitized) {
      finalUrls.push(`${UPLOADED_PERSONAL_FINE_PROVES_PREFIX}${f.filename}`);
      continue;
    }

    const finalBase = `${sanitized}${i + 1}.jpg`;
    const toAbs = path.join(PERSONAL_FINE_PROVES_DIR, finalBase);
    ...
    try {
      ...
      finalUrls.push(`${UPLOADED_PERSONAL_FINE_PROVES_PREFIX}${finalBase}`);
    } catch (e) {
      logger.error(
        `renameProveFilesByFineNumber falla en ${finalBase}: ${e}`
      );
      finalUrls.push(`${UPLOADED_PERSONAL_FINE_PROVES_PREFIX}${f.filename}`);
    }
  }
...
/** Basename del archivo a partir de su URL pública. */
export function proveBasenameFromUrl(url) {
  if (typeof url !== "string") return null;
  if (!url.startsWith(UPLOADED_PERSONAL_FINE_PROVES_PREFIX)) return null;
  return url
    .slice(UPLOADED_PERSONAL_FINE_PROVES_PREFIX.length)
    .replace(/^\/+/, "");
}
 ...
 * multa personal. Mismo flujo que las multas vehiculares/buques pero usando
 * el `fine_number` como prefijo de nombre.
 */
export function applyProveUpdatesForFine({
  ...
  ensurePersonalFineProvesDirSync();
  const root = path.resolve(PERSONAL_FINE_PROVES_DIR);
  const sanitized = sanitizeIdForFilename(fineNumber);
  ...
  for (const slot of compacted) {
    ...
    const fromAbs = path.join(PERSONAL_FINE_PROVES_DIR, slot.basename);
    const tmpName = `${randomUUID()}.tmp.jpg`;
    const tmpAbs = path.join(PERSONAL_FINE_PROVES_DIR, tmpName);
    ...
    try {
      if (!fs.existsSync(fromAbs)) {
        logger.warn(
          `applyProveUpdatesForFine (personal): archivo ${slot.basename} no existe.`
        );
        stagedNames.push(null);
        continue;
      }
      ...
      logger.error(
        `applyProveUpdatesForFine (personal) staging falla: ${e}`
      );
      stagedNames.push(null);
    }
  }
  ...
  for (let i = 0; i < compacted.length; i++) {
    ...
    if (!tmp) continue;
    const tmpAbs = path.join(PERSONAL_FINE_PROVES_DIR, tmp);
    ...
    try {
      ...
      logger.error(
        `applyProveUpdatesForFine (personal) commit falla: ${e}`
      );
    }
  }
  ...
  for (const baseName of toDelete) {
    ...
    const abs = path.join(PERSONAL_FINE_PROVES_DIR, baseName);
    if (!abs.startsWith(root)) continue;
    try {
      if (fs.existsSync(abs)) fs.unlinkSync(abs);
    } catch (e) {
      logger.warn(`applyProveUpdatesForFine (personal) delete: ${e}`);
    }
  }
