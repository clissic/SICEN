...
import {
  ALLOWED_CURRENT_FORECAST_OFFSETS,
  currentPointCacheKey,
  fetchOpenMeteoCurrentPoints,
} from "../utils/openMeteoCurrents.js";
...
function pruneCache(now) {
  ...
  const ttl = env.currentsCacheTtlMs;
...
/**
 * Corrientes superficiales con cache por punto (Open-Meteo Marine vía proxy).
 */
export async function getCurrentPoints({ points, forecastHoursOffset = 0 }) {
  ...
  if (!ALLOWED_CURRENT_FORECAST_OFFSETS.has(offset)) {
  ...
  for (let i = 0; i < normalized.length; i += 1) {
    ...
    const key = currentPointCacheKey(lat, lon, offset);
  ...
  if (toFetch.length > 0) {
    const fetched = await fetchOpenMeteoCurrentPoints(toFetch, offset);
    for (let j = 0; j < fetched.length; j += 1) {
      ...
      results[i] = point;
      cache.set(currentPointCacheKey(lat, lon, offset), { at: now, data: point });
    }
    pruneCache(now);
  }
