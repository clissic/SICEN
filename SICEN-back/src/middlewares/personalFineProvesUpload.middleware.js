...
const storage = multer.diskStorage({
  destination(_req, _file, cb) {
    ensurePersonalFineProvesDirSync();
    cb(null, PERSONAL_FINE_PROVES_DIR);
  },
...
const personalFineProvesUpload = multer({
  storage,
  limits: { fileSize: MAX_BYTES, files: MAX_FILES },
  fileFilter,
});

export function uploadPersonalFineProves(req, res, next) {
  personalFineProvesUpload.array(FIELD_NAME, MAX_FILES)(req, res, (err) => {
    if (!err) {
      next();
      return;
    }
...
export function uploadPersonalFineProveSlots(req, res, next) {
  personalFineProvesUpload.fields(PROVE_SLOT_FIELDS)(req, res, (err) => {
    if (!err) {
      next();
      return;
    }
