import { getWindPoints } from "../services/windProxy.service.js";
import { logger } from "../utils/logger.js";

function handleError(res, e, fallbackMsg) {
  const code = e.status || e.statusCode || 500;
  if (code === 400 || code === 403 || code === 404 || code === 409 || code === 502) {
...
export const windController = {
  async points(req, res) {
    try {
      ...
      const result = await getWindPoints({ points, forecastHoursOffset });
      return res.json({ ok: true, ...result });
    } catch (e) {
      return handleError(res, e, "No se pudo obtener el viento.");
    }
  },
};
