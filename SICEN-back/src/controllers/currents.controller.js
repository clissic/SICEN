import { getCurrentPoints } from "../services/currentsProxy.service.js";
...
export const currentsController = {
  async points(req, res) {
    try {
      const { points, forecastHoursOffset } = req.body ?? {};
      const result = await getCurrentPoints({ points, forecastHoursOffset });
      return res.json({ ok: true, ...result });
    } catch (e) {
      return handleError(res, e, "No se pudieron obtener las corrientes.");
    }
  },
};
