...
Este documento describe la **paleta y los tokens CSS** usados en la interfaz (`public/assets/styles/styles.css`), alineados con el **modo claro/oscuro de Bootstrap 5** (`data-bs-theme` en `<html>`).

---

## Cómo se activa el tema

- En **`src/views/layouts/main.handlebars`**, `<html>` lleva por defecto **`data-bs-theme="light"`**.
- **`public/assets/js/index.js`** lee `localStorage` (`sistemaCentinela.theme`: `light` | `dark`; migra desde una clave de tema antigua si aún existe) o `prefers-color-scheme`, y aplica:
...
Definidos en `styles.css`: parte en **`:root`** (modo claro implícito) y sobrescritura en **`html[data-bs-theme="dark"]`**.
...
- `--control-bg` / `--control-text` / `--control-muted-bg` / `--placeholder`: ver `styles.css`
