...
| **Gente de mar (`/base-gente-mar`)** | Hub con tarjetas: **INGRESAR PERSONA** → `/base-gente-mar/nuevo` (`SeafarerCreateForm.jsx`, alta con datos iniciales y documento). **CONSULTAR Y MODIFICAR** → `/base-gente-mar/todos` (`SeafarerConsultPage.jsx`): búsqueda por **tipo y número de documento** (`GET /api/seafarers/by-document`); ficha con datos tabulados, **títulos** (subdocumentos en `titles` + catálogo en colección `titles` vía `/api/titles`), **licencias** poseídas (`heldLicenses` referenciando entradas `kind: license` del catálogo `/api/licences`) y filas **históricas** aplanadas desde los buckets `licenses.recreational|comercial|special`; además **cursos**, **sanciones** y **observaciones** (solo altas desde esta pantalla). En títulos y licencias de catálogo se exigen fechas de emisión y vencimiento coherentes; las tablas de **títulos** y **licencias** muestran la columna **Renovaciones** (`renewalsCount`; en filas solo históricas puede mostrarse «—»); al **editar**, un checkbox **Renovación** (desmarcado por defecto) envía `isRenewal: true` en el cuerpo del `PATCH` para que el servidor **incremente en 1** el contador. Tras un guardado exitoso de títulos o licencias la vista hace **scroll suave** a la sección correspondiente (`scrollElementIntoViewById` en `scrollPageToTop.js`; anclas `seafarer-consult-titles` y `seafarer-consult-licenses` en `SeafarerConsultSections.jsx`). **METADATOS** → `/base-gente-mar/editar` (`SeafarerMetadataPage.jsx`): ABM paginado de catálogos **títulos** y **licencias** (`SeafarerMetadataAddModal.jsx`) y listados de definiciones de **cursos** y **sanciones** (`GET /api/seafarers/metadata/courses` y `.../sanctions`). **Eliminar** → `/base-gente-mar/eliminar` (placeholder; solo **admin** / **superAdmin**). Constantes de formulario y filas de tabla: `SICEN-front/src/constants/seafarerConsult.js`. |
